void Encoders()
{
  ...
  if (lencold!=lencnew) {
    leftmotor.encoder+=leftmotor.speed/abs(leftmotor.speed);                // if old does not equal new then add or subtract 1 depending on motor direction
  }

  if (rencold!=rencnew) {
    rightmotor.encoder+=rightmotor.speed/abs(rightmotor.speed);                // if old does not equal new then add or subtract 1 depending on motor direction
  }
}
